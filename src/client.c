...
typedef struct urlinfo_t {
  char *hostname;           // hostname is everything before the colon, excluding `http://` or `https://`
  char *port;               // port is the number between the `:` and `/`
  char *path;               // path is everything after `/`
} urlinfo_t;
